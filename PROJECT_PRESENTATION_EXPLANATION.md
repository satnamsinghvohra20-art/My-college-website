...
#### Q9: How did you evaluate the performance of your Machine Learning classification model?
> **Answer:** *"Ma'am, in `pages/analytics.html`, we built a live Confusion Matrix & Model Evaluation HUD evaluated over $n = 1,000$ historical student test records. At the default decision cutoff ($\theta = 0.50$), our Logistic Sigmoid Defaulter Classifier achieves **98.3% Accuracy**, **92.2% Precision**, **96.6% Recall (Sensitivity)**, and an **F1-Score of 0.943** with an **AUC-ROC of 0.982**. Examiners can slide the decision threshold in real time to observe the classical Precision-Recall tradeoff."*

#### Q10: What is Module 24 (AI Placement & CTC Forecaster), and what insights does it provide?
> **Answer:** *"Ma'am, in `pages/placement.html`, we engineered an empirical multi-variable regression model forecasting campus recruitment salary packages based on student CGPA, coding assessment score, industry certifications, and internship months: $\text{CTC} = 0.52(\text{CGPA}) + 0.045(\text{Code}) + 0.55(\text{Certs}) + 0.35(\text{Intern}) - 1.20$. It classifies students into Tier 1 Dream (Deloitte, Barclays, Morgan Stanley - 8 to 14 LPA), Tier 2 Core IT (Capgemini, LTI Mindtree - 5 to 8 LPA), and Tier 3 Mass Recruiters (TCS, Wipro), while providing dynamic prescriptive skill gap recommendations."*

---

## 🎬 Section 4: 5-Minute Live Demonstration Flow

When presenting live to your teacher, follow this crisp sequence on [http://localhost:8080](http://localhost:8080):

1. **Step 1: Homepage & Examiner Showcase Tour (`index.html`)**
   - Show the official crest and NAAC 'A' grade badge.
   - Click the golden **"🎯 Viva Showcase Tour"** button in the topbar to launch the automated interactive walkthrough.
   - Click the language toggle to switch to **मराठी** (*मराठी भाषा संवर्धन*).
   - Test **ChandiBot** voice inquiry using speech recognition (`🎤`).
2. **Step 2: Student ERP Portal (`portal.html`)**
   - Point out the **Circular SVG Attendance Gauge** (86.4% Good Standing vs 75% threshold).
   ...
   - Click **Generate Railway Concession** to display the verified print-ready Central Railway pass.
   - Open the **Classroom Dynamic QR Projector HUD** and show the 10-second countdown timer rotating the cryptographic token.
3. **Step 3: Data Science & AI Predictive Analytics Hub (`analytics.html`)**
   - Show the **Ordinance 0.119 Defaulter Forecaster**: adjust the attendance slider to 68% to trigger the amber warning.
   - Demonstrate the **Confusion Matrix HUD**: adjust the decision threshold slider ($\theta$) to show live Precision/Recall recalculation.
   - Show the **SGPA Forecaster**: tweak CIA marks and study hours to observe the predicted grade.
   - Click **Re-Calculate Centroids (k=4)** on the HTML5 Canvas to watch K-Means clusters converge.
   - Click **Export Cohort Dataset (CSV)** to show the instant data download.
4. **Step 4: AI Exam Seating & Anti-Cheating Allocator (`exam-seating.html`)**
   - Switch venues across Kundnani Auditorium, Lab 301, and Block 204 to demonstrate real-time 2D floorplan capacity meters.
...
5. **Step 5: Corporate Placements & CTC Forecaster (`placement.html`)**
   - Demonstrate **Module 24**: adjust CGPA and coding score sliders to see the forecasted package (e.g. ₹8.45 LPA) and Tier 1 Dream Company matches.
6. **Step 6: Printable Executive Defense Cheat Sheet**
   - Open [`docs/VIVA_VOCE_EXECUTIVE_CHEAT_SHEET.pdf`](file:///c:/Users/satna/Downloads/chm%20clone/docs/VIVA_VOCE_EXECUTIVE_CHEAT_SHEET.pdf) showing the 1-page summary of all key formulas, benchmarks, and rapid-fire answers.
